...
USER_REAL_NAME="Panchaleswar Nayak"
# Role of the user
GLOBAL_ROLE=user
...
USER_ID=`curl -s -u $ADMIN_BEARER_TOKEN $RANCHER_API_ENDPOINT/user -H 'content-type: application/json' --data-binary '{"me":false,"mustChangePassword":false,"type":"user","username":"'$USER_NAME'","password":"'$PASSWORD'","name":"'$USER_REAL_NAME'"}' --insecure | jq -r .id`
curl -s -u $ADMIN_BEARER_TOKEN $RANCHER_ENDPOINT/globalrolebinding -H 'content-type: application/json' --data-binary '{"type":"globalRoleBinding","globalRoleId":"'$GLOBAL_ROLE'","userId":"'$USER_ID'"}' --insecure
...
LOGIN_RESPONSE=`curl -s $RANCHER_API_ENDPOINT-public/localProviders/local?action=login -H 'content-type: application/json' --data-binary '{"username":"'$USER_NAME'","password":"'$PASSWORD'"}' --insecure`
